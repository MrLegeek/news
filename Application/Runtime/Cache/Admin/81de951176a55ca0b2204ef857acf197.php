...
<article class="page-container">
	<form class="form  form-horizontal" method="post"enctype="multipart/form-data" action="<?php echo U('News/newsEditSave');?>" id="form-article-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($newsinfo["title"]); ?>" placeholder="新闻标题"  id="title" name="title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>标题颜色：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="标题颜色" value="<?php echo ($newsinfo["title_color"]); ?>" id="title_color" name="title_color">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">新闻简略：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($newsinfo["brief"]); ?>" placeholder="新闻简略" id="brief" name="brief">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" name="role_id">
				  <?php if(is_array($roleInfo)): foreach($roleInfo as $key=>$vo): if($vo['id'] == $newsinfo['role_id']): ?><option value="<?php echo ($vo["id"]); ?>" selected>├<?php echo ($vo["role_name"]); ?></option>
						<?php else: ?>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" name="cate_id">
				  <?php if(is_array($cateInfo)): foreach($cateInfo as $key=>$vo): if($vo['cate_id'] == $newsinfo['cate_id']): ?><option value="<?php echo ($vo["cate_id"]); ?>" selected>├<?php echo ($vo["cate_name"]); ?></option>
						<?php else: ?>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($newsinfo["author"]); ?>" placeholder="" disabled id="author" name="author">
			</div>
		</div>
				...
				<div class="uploader-thum-container">
					...
					<div id="imgShow">
						...
						<div class="image_container" style="width: 200px;height: 200px;border: 1px solid #FF0000" >
							<img  id="ImgPreview"  src="<?php echo ($newsinfo['news_small_img']); ?>" style="width: 100%;height: 100%" >
						</div>
					</div>
				</div>
			</div>
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<div class="check-box">
					<label for="top-1"><input type="radio" name="is_top" id="top-1"  checked value="0"
						<?php switch($newsinfo['is_top']): case "0": ?>checked<?php break;?>
							<?php case "1": break; endswitch;?> >
						否</label>
				</div>
				<div class="check-box">
					<label for="top-2"><input type="radio" name="is_top" id="top-2"   value="1"
						<?php switch($newsinfo['is_top']): case "0": break;?>
							<?php case "1": ?>checked<?php break; endswitch;?> >
						是</label>
				</div>
			</div>
		</div>
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<div class="check-box">
					...
					<label for="checksize-1">
						...
						<?php switch($newsinfo['is_rotaion']): case "0": ?>checked<?php break;?>
							<?php case "1": break; endswitch;?> >
						否</label>
				</div>
				<div class="check-box">
					...
					<?php switch($newsinfo['is_rotaion']): case "1": ?>checked<?php break;?>
						<?php case "0": break; endswitch;?> >
					是</label>
				</div>
			</div>
		</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				...
				<a  href="<?php echo U('News/news_list');?>" class="btn btn-secondary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 返回</a>
			</div>
		</div>
	</form>
</article>
...
//选择标题颜色
		...
		var uploader = WebUploader.create({
			...
			// 文件接收服务端。
			server: "<?php echo U('News/ajax_upload');?>",
