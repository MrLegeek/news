...
class localController extends IndexController
{
    public function local_list(){
        ...
        $news=D('news');
        $where='role_id='.$role_id .' and del_flag=0'.' and news.local_id !=0';
        $sql=" SELECT news.* ,local.local_name FROM `news` inner JOIN local on news.local_id=local.local_id WHERE ($where)";
