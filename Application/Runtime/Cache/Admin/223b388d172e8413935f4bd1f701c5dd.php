...
<div style="float: right;margin: 0 10px"> <p><a class="btn btn-default"  href="javascript:" onclick="self.location=document.referrer;"  style="color: #3389DE; font-size: 20px;font-weight: bold;cursor: pointer">返回上一页</a></p></div>
        <nav style="height: 40px">
            <?php if($lmsg[status]== 0 ): ?><div style="float: left;margin: 0 10px" id="success"> <p><a class="btn btn-default" onClick="submit_success(this,'/news/index.php/Admin/Lmsg/submit_success/id/<?php echo ($lmsg["id"]); ?>','<?php echo ($lmsg["id"]); ?>')" href="javascript:;" style="color: #3389DE; font-size: 20px ;font-weight: bold;margin: 20px 0px;cursor: pointer">审核通过</a></p></div>
                <div style="float: left;" id="fail"> <p><a  class="btn btn-default" onClick="submit_fail(this,'/news/index.php/Admin/Lmsg/submit_fail/id/<?php echo ($lmsg["id"]); ?>','<?php echo ($lmsg["id"]); ?>')" href="javascript:;" style="color:red; font-size: 20px ;font-weight: bold;margin: 20px 0px;cursor: pointer">审核不通过</a></p></div>
                <?php else: ?>
                <?php if($lmsg[status] == 1 ): ?><div style="float: left"> <p><a onClick="submit_success2(this,'/news/index.php/Admin/Lmsg/submit_success/id/<?php echo ($lmsg["id"]); ?>','<?php echo ($lmsg["id"]); ?>')" href="javascript:;" style="color: #3389DE; font-size: 20px ;font-weight: bold;margin: 20px 0px;cursor: pointer">审核已经通过</a></p></div>
                    <?php else: ?>
            ...
            <?php if($lmsg["verifier"] == null): ?><div  style=" margin:0 auto;color: red; font-size: 20px ;font-weight: bold; float:right">当前审核人:<span name="verifier" id="verifier"><?php echo (session('username')); ?></span></div>
                <?php else: ?>
                <div name="verifier" style=" margin:0 auto;color: red; font-size: 20px ;font-weight: bold; float:right">审核人:<span><?php echo ($lmsg["verifier"]); ?></span></div><?php endif; ?>
        </nav>
